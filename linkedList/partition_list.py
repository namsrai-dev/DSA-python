...
class LinkedList:
    ...
    def partition_list(self, x):
        ...
        for idx in range(self.length):
            ...
            if pointer.value < x:
                if dummy1 == None:
                    print("adding new", pointer.value)
                    dummy1 = Node(pointer.value)
                    prev1 = dummy1
                else:
                    current = dummy1
                    print("adding ", pointer.value)
                    while current.next is not None:
                        current = current.next
                    current.next = Node(pointer.value)
                    prev1 = current.next
            else:
                if dummy2 == None:
                    print("adding new", pointer.value)
                    dummy2 = Node(pointer.value)
                else:
                    current = dummy2
                    print("adding ", pointer.value)
                    while current.next is not None:
        ...
        if not dummy1 == None:
            print("dummy1", linkedlist_to_list(dummy1))
            # prev1.next = dummy2.next
        if not dummy2 == None:
            # prev1.next = dummy2.next
            print("dummy2", linkedlist_to_list(dummy2))

        if dummy1 == None and dummy2 == None:
            self.head = None
        elif dummy2 == None:
            self.head = dummy1
        elif dummy1 == None:
